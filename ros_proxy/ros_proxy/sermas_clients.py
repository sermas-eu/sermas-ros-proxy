...
class SermasApiClient:
    ...
    def retrieve_token(self):
        ...
        try:
            ...
            response_json = response.json()
            logging.info("Token response: %s" % str(response_json))
...
class SermasMQTTClient:
    ...
    def publish(self, topic, message):
        ...
        self.client.publish(f"app/{self.app_id}/{topic}", json.dumps(message))
        logging.debug(f"[MQTT] published {json.dumps(message)}")
