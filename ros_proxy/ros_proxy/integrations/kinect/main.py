...
class IntentDetection(IntegrationBaseClass):
  def __init__(self, ros_node):
    ...
    self.sermas_device_status_topic = 'todo'
  ...
  def check_status(self):
    ...
    self.ros_node.get_logger().info("Device status: %s" % ("available" if self.data_available else "unavailable"))
    # publish status
    d = { "status": "available" if self.data_available else "unavailable" }
    # self.ros_node.mqtt_client.publish(self.sermas_device_status_topic, d)
